    ...
    : QMainWindow(parent)
{
    ...
    ui.lineEdit->setPlaceholderText((u8"输入歌手或歌曲"));
...
inline void MusicPlayer::init_else()
{
    ...
    if (FileName.isEmpty())
    {
        FileName1 = ":/MusicPlayer/image/background/bg8.jpg";
    }
    else
    {
        FileName1 = FileName;
    ...
    ui.pushButton_10->setStyleSheet("QPushButton"
        "{"
        "background-image: url(:/MusicPlayer/image/image-hover/music-hover.png);"
        ...
        "}"
        "QPushButton:pressed{"
        "background-image: url(:/MusicPlayer/image/image/music.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
...
inline void MusicPlayer::init_win()
{
    ...
    next->setToolTip((u8"下一首"));
    next->setIcon(QIcon(":/MusicPlayer/image/next1.png"));
    connect(next, &QWinThumbnailToolButton::clicked, this, &MusicPlayer::action9_slot);//下一首->action10_slot槽
    pre->setToolTip((u8"上一首"));
    pre->setIcon(QIcon(":/MusicPlayer/image/pre1.png"));
    connect(pre, &QWinThumbnailToolButton::clicked, this, &MusicPlayer::action10_slot);//上一首->action9_slot槽
    pause->setToolTip((u8"暂停"));
    pause->setIcon(QIcon(":/MusicPlayer/image/bf13.png"));
    connect(pause, &QWinThumbnailToolButton::clicked, this, &MusicPlayer::on_pushButton_2_clicked);//播放/暂停->on_pushButton_2_clicked槽
...
inline void MusicPlayer::init_Connect() {


    connect(music, &QMediaPlayer::metaDataAvailableChanged, this, &MusicPlayer::updateInfo);//音乐歌名改变->更新显示文本
    ...
    connect(playlist, &QMediaPlaylist::currentIndexChanged, this, &MusicPlayer::updatalistwidget_3);//本地音乐播放列表位置改变->更新listWidget_3的位置
    connect(playlist_2, &QMediaPlaylist::currentIndexChanged, this, &MusicPlayer::updatalistwidget_2);//我喜欢播放列表位置改变->更新listWidget_2的位置
    connect(playlist_3, &QMediaPlaylist::currentIndexChanged, this, &MusicPlayer::updatalistwidget);//我的收藏播放列表位置改变->更新listWidget的位置
    connect(action1, &QAction::triggered, this, &MusicPlayer::action1_slot);//添加至本地音乐动作点击->执行action1_slot槽
    connect(action2, &QAction::triggered, this, &MusicPlayer::action2_slot);//添加我喜欢动作点击->执行action2_slot槽
    connect(action3, &QAction::triggered, this, &MusicPlayer::action3_slot);//添加我的收藏动作点击->执行action3_slot槽
    connect(action4_1, &QAction::triggered, this, &MusicPlayer::action4_1_slot);//透明度加动作点击->执行action4_1_slot槽
    connect(action4_2, &QAction::triggered, this, &MusicPlayer::action4_2_slot);//透明度减动作点击->执行action4_2_slot槽
    connect(action5_1, &QAction::triggered, this, &MusicPlayer::action5_1_slot);//默认皮肤1动作点击->执行action5_1_slot槽
    connect(action5_2, &QAction::triggered, this, &MusicPlayer::action5_2_slot);//默认皮肤2动作点击->执行action5_2_slot槽
    connect(action5_3, &QAction::triggered, this, &MusicPlayer::action5_3_slot);//自定义皮肤动作点击->执行action5_3_slot槽
    connect(action5_4, &QAction::triggered, this, &MusicPlayer::action5_4_slot);//3
    connect(action6, &QAction::triggered, this, &MusicPlayer::action6_slot);//关于动作点击->执行action6_slot槽
    connect(action7, &QAction::triggered, this, &MusicPlayer::action7_slot);//使用介绍动作点击->执行action7_slot槽
    connect(action8, &QAction::triggered, this, &MusicPlayer::action8_slot);//退出动作点击->执行action8_slot槽
    connect(ClearALL_1, &QAction::triggered, this, &MusicPlayer::ClearALL_1_slot);//清除本地音乐动作点击->执行ClearALL_1_slot槽
    connect(ClearALL_2, &QAction::triggered, this, &MusicPlayer::ClearALL_2_slot);//清除我喜欢动作点击->执行ClearALL_2_slot槽
    connect(ClearALL_3, &QAction::triggered, this, &MusicPlayer::ClearALL_3_slot);//清除我的收藏动作点击->执行ClearALL_3_slot槽
    connect(ClearALL_4, &QAction::triggered, this, &MusicPlayer::ClearALL_4_slot);//清除所有音乐动作点击->执行ClearALL_4_slot槽
    connect(action13, &QAction::triggered, this, &MusicPlayer::action8_slot);//退出动作点击->执行action8_slot槽
    connect(action9, &QAction::triggered, this, &MusicPlayer::action9_slot);//播放上一首动作点击->执行action9_slot槽
    connect(action10, &QAction::triggered, this, &MusicPlayer::action10_slot);//播放下一首动作点击->执行action10_slot槽
    connect(action11, &QAction::triggered, this, &MusicPlayer::action11_slot);//暂停动作点击->执行action11_slot槽
    connect(action11_1, &QAction::triggered, this, &MusicPlayer::action11_1_slot);//播放动作点击->执行action11_1_slot槽
    connect(action12_1, &QAction::triggered, this, &MusicPlayer::action12_1_slot);//顺序播放动作点击->执行action12_1_slot槽
    connect(action12_2, &QAction::triggered, this, &MusicPlayer::action12_2_slot);//随机播放动作点击->执行action12_2_slot槽
    connect(action12_3, &QAction::triggered, this, &MusicPlayer::action12_3_slot);//单曲循环播放动作点击->执行action12_3_slot槽
    connect(ui.verticalSlider, &QSlider::valueChanged, this, &MusicPlayer::changeVolume);//音量进度条置的改变->音量改变
    connect(music, &QMediaPlayer::stateChanged, this, &MusicPlayer::music_stateChang);//音乐的播放状态改变->执行music_stateChang槽
    connect(Music, &Dialog::Data, this, &MusicPlayer::Data_slot);//接收极简播放器的数据
    connect(detila, &QAction::triggered, this, &MusicPlayer::detila_slot);//歌曲详情动作点击->执行detila_slot槽
    connect(playlist, &QMediaPlaylist::playbackModeChanged, this, &MusicPlayer::PlaylistModel_slot);//播放列表模型改变->执行MusicPlayer::PlaylistModel_slot槽
    connect(location, &QAction::triggered, this, &MusicPlayer::on_pushButton_7_clicked);//定位动作点击->执行on_pushButton_7_clicked槽
    connect(ClearHistory, &QAction::triggered, this, &MusicPlayer::ClearHistory_Slot);

}
inline void MusicPlayer::init_tooltip()
{
    ui.horizontalSlider->setToolTip((u8"播放进度"));
    ui.pushButton->setToolTip((u8"添加歌曲"));
    ui.pushButton_2->setToolTip((u8"暂停"));
    ui.pushButton_3->setToolTip((u8"下一首"));
    ui.pushButton_4->setToolTip((u8"上一首"));
    ui.pushButton_5->setToolTip((u8"设置"));
    ui.pushButton_6->setToolTip((u8"更换皮肤"));
    ui.pushButton_7->setToolTip((u8"定位"));
    ui.pushButton_8->setToolTip((u8"音量"));
    ui.pushButton_9->setToolTip((u8"顺序播放"));
    ui.pushButton_10->setToolTip((u8"本地音乐"));
    ui.pushButton_11->setToolTip((u8"我喜欢"));
    ui.pushButton_12->setToolTip((u8"我的收藏"));
    ui.pushButton_13->setToolTip((u8"切换至极简模式/双击"));
    ui.pushButton_15->setToolTip((u8"隐藏播放列表"));
    ui.pushButton_17->setToolTip((u8"关闭"));
    ui.pushButton_16->setToolTip((u8"最小化"));

}
inline void  MusicPlayer::init_icon()
{
    ...
    ui.pushButton_15->setIcon(QIcon(":/MusicPlayer/image/image/left.png"));
    ui.pushButton_15->setStyleSheet(HideListStyle());
    ui.pushButton_9->setIconSize(QSize(32, 32));//播放模式
    ui.pushButton_9->setIcon(QIcon(":/MusicPlayer/image/image/loop.png"));
    ui.pushButton_9->setStyleSheet(LoopStyle());
    ui.pushButton_14->setIconSize(QSize(32, 32));//我喜欢
    ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/image/Ilike.png"));
    ...
    ui.pushButton_8->setIcon(QIcon(":/MusicPlayer/image/image/music-voice.png"));


}
inline void  MusicPlayer::init_action()
{

    action1 = new QAction(this);//添加至本地音乐
    action2 = new QAction(this);//添加至我喜欢
    action3 = new QAction(this);//添加至我的收藏
    action1->setIcon(QIcon(":/MusicPlayer/image/music_32px_1125557_easyicon.net.png"));
    action1->setText((u8"添加至本地音乐"));
    action1->setShortcut(QKeySequence("Ctrl+O"));
    action2->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1101681_easyicon.net.png"));
    action2->setText((u8"添加至我喜欢"));
    action2->setShortcut(QKeySequence("Ctrl+A"));
    action3->setIcon(QIcon(":/MusicPlayer/image/list_32px_1074296_easyicon.net.png"));
    action3->setText((u8"添加至我的收藏"));
    ...
    add->setIcon(QIcon(":/MusicPlayer/image/add.png"));
    add->setText((u8"添加歌曲"));
    add->setMenu(menu1);
    menu1->setStyleSheet(
        MenuStyle()
    ...
    location->setText((u8"定位"));
    location->setIcon(QIcon(":/MusicPlayer/image/location_32px_1074876_easyicon.net.png"));
    ...
    action4->setIcon(QIcon(":/MusicPlayer/image/ooopic_1500874104.png"));
    action4->setText((u8"设置透明度           "));

    action4_1 = new QAction(this);//透明度加
    action4_2 = new QAction(this);//透明度减
    action4_1->setIcon(QIcon(":/MusicPlayer/image/add.png"));
    action4_1->setText((u8"+5%"));
    action4_1->setShortcut(QKeySequence("Ctrl++"));
    action4_2->setIcon(QIcon(":/MusicPlayer/image/sub.png"));
    action4_2->setText((u8"-5%"));
    ...
    action5->setIcon(QIcon(":/MusicPlayer/image/pf.png"));
    action5->setText((u8"设置皮肤"));

    action5_1->setIcon(QIcon(":/MusicPlayer/image/pf2.png"));
    action5_1->setText((u8"绿色"));

    action5_2->setIcon(QIcon(":/MusicPlayer/image/pf5.png"));
    action5_2->setText((u8"动漫"));
    action5_4->setIcon(QIcon(":/MusicPlayer/image/pf.png"));
    action5_4->setText((u8"蓝色"));
    action5_3->setIcon(QIcon(":/MusicPlayer/image/pf4.png"));
    action5_3->setText((u8"自定义"));
    ...
    action6->setIcon(QIcon(":/MusicPlayer/image/ooopic_1500873230.png"));
    action6->setText((u8"关于"));
    action7->setIcon(QIcon(":/MusicPlayer/image/ooopic_1500873272.png"));
    action7->setText((u8"使用介绍"));
    ClearALL->setIcon(QIcon(":/MusicPlayer/image/edit_clear_locationbar_ltr_32px_539686_easyicon.net.png"));
    ClearALL->setText((u8"清除"));

    ClearALL_1->setIcon(QIcon(":/MusicPlayer/image/Music_32px_1144946_easyicon.net.png"));
    ClearALL_1->setText((u8"清除本地音乐列表"));
    ClearALL_1->setShortcut(QKeySequence("Ctrl+M"));
    ClearALL_2->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1170275_easyicon.net.png"));
    ClearALL_2->setText((u8"清除我喜欢列表"));
    ClearALL_2->setShortcut(QKeySequence("Ctrl+N"));
    ClearALL_3->setIcon(QIcon(":/MusicPlayer/image/list_32px_1142913_easyicon.net.png"));
    ClearALL_3->setText((u8"清除我的收藏"));
    ClearALL_3->setShortcut(QKeySequence("Ctrl+B"));
    ClearALL_4->setIcon(QIcon(":/MusicPlayer/image/playlist_27.690544412607px_1187707_easyicon.net.png"));
    ClearALL_4->setText((u8"清除所有列表"));
    ClearALL_4->setShortcut(QKeySequence("Ctrl+V"));
    ClearHistory->setIcon(QIcon(":/MusicPlayer/image/nextbo.png"));
    ClearHistory->setText((u8"清除历史记录"));
    ...
    action8->setIcon(QIcon(":/MusicPlayer/image/tc.png"));
    action8->setText((u8"退出"));
    ...
    action9->setIcon(QIcon(":/MusicPlayer/image/pre1.png"));
    action9->setText((u8"播放上一首"));

    action10 = new QAction(this);//播放下一首
    action10->setIcon(QIcon(":/MusicPlayer/image/next1.png"));
    action10->setText((u8"播放下一首"));

    action11 = new QAction(this);//暂停
    action11->setIcon(QIcon(":/MusicPlayer/image/zangting.png"));
    action11->setText((u8"暂停"));

    action11_1 = new QAction(this);//播放
    action11_1->setIcon(QIcon(":/MusicPlayer/image/bf2.png"));
    action11_1->setText((u8"播放"));


    action12 = new QAction(this);//播放模式
    action12->setIcon(QIcon(":/MusicPlayer/image/moshi1.png"));
    action12->setText((u8"播放模式"));
    action12_1 = new QAction(this);//顺序播放
    action12_1->setIcon(QIcon(":/MusicPlayer/image/player-icons_32px_1137007_easyicon.net.png"));
    action12_1->setText((u8"顺序播放"));
    action12_2 = new QAction(this);//随机播放
    action12_2->setIcon(QIcon(":/MusicPlayer/image/player-icons_32px_1137005_easyicon.net.png"));
    action12_2->setText((u8"随机播放"));
    action12_3 = new QAction(this);//单曲循环
    action12_3->setIcon(QIcon(":/MusicPlayer/image/player-icons_32px_1137006_easyicon.net.png"));
    action12_3->setText((u8"单曲循环"));
    ...
    action13->setIcon(QIcon(":/MusicPlayer/image/tc.png"));
    action13->setShortcut(QKeySequence("Ctrl+Q"));
    action13->setText((u8"退出"));
    detila = new QAction(this);//歌曲详情
    detila->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    detila->setText((u8"歌曲信息"));

}
inline void  MusicPlayer::init_list()
{
    ...
    while (query.next())//初始化本地音乐播放列表
    {
        ...
        if (!Name.isEmpty() && !FileName.isEmpty())
        {
            ...
            item1->setIcon(QIcon(":/MusicPlayer/image/Music_32px_1144946_easyicon.net.png"));
            item1->setText(Name);
    ...
    while (query.next())//初始化我喜欢播放列表
    {
        ...
        if (!Name.isEmpty() && !FileName.isEmpty())
        {
            ...
            item4->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1170275_easyicon.net.png"));
            item4->setText(Name);
    ...
    while (query.next())//初始化我的收藏播放列表
    {
        ...
        if (!Name.isEmpty() && !FileName.isEmpty())
        {
            ...
            item5->setIcon(QIcon(":/MusicPlayer/image/music_not_25.570093457944px_1171234_easyicon.net.png"));
            item5->setText(Name);
...
inline QString  MusicPlayer::MusicListStyle() {
...
inline QString  MusicPlayer::MenuStyle()
{
    return     " QMenu {"
        " background: white; "
        "border-radius:2px;"
...
inline QString  MusicPlayer::PlayStyle()
{

    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/play.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/play-hover.png);"
...
inline QString  MusicPlayer::PaseStyle()
{


    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/pase.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/pase-hover.png);"
...
inline QString  MusicPlayer::IlikeStyle()
{


    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/Ilike.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/Ilike-hover.png);"
...
inline QString MusicPlayer::Ilike1Style()
{


    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/Ilike1.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/Ilike1-hover.png);"
...
inline QString MusicPlayer::ShowListStyle()
{
    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/right.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/right-hover.png);"
...
inline QString MusicPlayer::HideListStyle()
{
    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/left.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/left-light.png);"
...
inline QString MusicPlayer::RandomStyle()
{

    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/play-random.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/play-random-hover.png);"
...
inline QString MusicPlayer::LoopStyle()
{


    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/loop.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/loop-hover.png);"
...
inline QString MusicPlayer::LoopOneStyle()
{


    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/loop-one.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/loop-one-hover.png);"
...
inline QString MusicPlayer::VoiceStyle()
{

    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/music-voice.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/music-voice-hover.png);"
...
inline QString MusicPlayer::NoVoiceStyle()
{

    return "QPushButton"
        " {"
        "background-image:url(:/MusicPlayer/image/image/none-music.png);"
        "   background-repeat:no-repeat;"
        "  background-position:center center;"
        "   border:none;"
        ...
        "  background-repeat:no-repeat;"
        "   background-position:center center;"
        "background-image:url(:/MusicPlayer/image/image-hover/none-music-hover.png);"
...
void MusicPlayer::updateInfo()//更新文本
{
    ...
    {
        ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/image/Ilike1.png"));
        ui.pushButton_14->setStyleSheet(Ilike1Style());
        ui.pushButton_14->setToolTip((u8"我喜欢"));
    }
    else if (music->playlist() == playlist_3)
    {

        ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/image/music.png"));
        ui.pushButton_14->setStyleSheet(MusicListStyle());
        ui.pushButton_14->setToolTip((u8"我的收藏"));
    ...
    else if (music->playlist() == playlist_4)
    {
        ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/image/music.png"));
        ui.pushButton_14->setStyleSheet(MusicListStyle());
        ui.pushButton_14->setToolTip((u8"播放历史"));
    }
    else
    {
        //判断该歌曲是不是我喜欢歌曲
        ...
        else
        {
            ...
            ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/image/Ilike.png"));
            ui.pushButton_14->setStyleSheet(IlikeStyle());
            ui.pushButton_14->setToolTip((u8"添加至我喜欢"));
...
void MusicPlayer::updatePosition(qint64 position)//更新播放位置
{
    ui.horizontalSlider->setValue(position);
    ui.label_2->setText(Time(position) + "/" + Time(music->duration()));
}
void MusicPlayer::updateDuration(qint64 duration)//更新播放进度
{
...
void MusicPlayer::pushbuttonstyle_5()
{
    ...
    ui.pushButton_11->setStyleSheet(
        ...
        " QPushButton"
        "{"
        "background-image: url(:/MusicPlayer/image/image/like.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
        "border:none;"
        ...
        "background-image: url(:/MusicPlayer/image/image-hover/like-hover.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
    ...
    ui.pushButton_12->setStyleSheet("QPushButton"
        "{"
        "background-image: url(:/MusicPlayer/image/image/list.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
        "border:none;"
        ...
        "background-image: url(:/MusicPlayer/image/image-hover/list-hover.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
    ...
    ui.pushButton_22->setStyleSheet("QPushButton"
        "{"
        "background-image: url(:/MusicPlayer/image/image-hover/play-resently-hover.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
        "border:none;"
        ...
        "}"
        "QPushButton:pressed{"
        "background-image: url(:/MusicPlayer/image/image/play-resently.png);"
        "background-repeat:no-repeat;"
        "background-position:center center;"
...
void MusicPlayer::pushbuttonstyle()//效果设置  本地音乐按钮显示白色背景
{
...
void MusicPlayer::pushbuttonstyle_2()//效果设置  我喜欢列表按钮显示白色背景
{
    ui.pushButton_10->setStyleSheet("QPushButton"
        "{"
        "background-image:url(:/MusicPlayer/image/image/music.png);"
...
void MusicPlayer::pushbuttonstyle_3()//效果设置  我的收藏按钮显示白色背景
{
...
void MusicPlayer::pushbuttonstyle_4()//效果设置  所有按钮背景都不变白色
{
...
void MusicPlayer::on_pushButton_10_clicked()//跳至本地音乐
{
...
void MusicPlayer::on_pushButton_11_clicked()//跳至自我喜欢
{
...
void MusicPlayer::on_pushButton_12_clicked()//跳至我的收藏
{
    ui.listWidget_2->setSelectionMode(QListWidget::SingleSelection);
    ui.listWidget_3->setSelectionMode(QListWidget::SingleSelection);
...
void MusicPlayer::on_pushButton_22_clicked()//历史记录
{
    ui.listWidget->setSelectionMode(QListWidget::SingleSelection);
    ui.listWidget_2->setSelectionMode(QListWidget::SingleSelection);
    ui.listWidget_3->setSelectionMode(QListWidget::SingleSelection);
...
void MusicPlayer::detila_slot()//歌曲详情
{
    ListFocus();
...
void MusicPlayer::ClearALL_1_slot()//清除本地音乐
{
    int values = QMessageBox::information(this, (u8"清除本地音乐列表"), (u8"确定清除本地音乐列表?                              ."), QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
...
/**清除历史记录
 * @brief MusicPlayer::ClearHistory_Slot
 */
void MusicPlayer::ClearHistory_Slot() {

    int values = QMessageBox::information(this, (u8"清除历史记录"), (u8"确定清除所有历史记录?."), QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    if (values == QMessageBox::Yes)
    {
...
void MusicPlayer::ClearALL_2_slot()//清除我喜欢列表
{
    int values = QMessageBox::information(this, (u8"清除我喜欢列表"), (u8"确定清除我喜欢列表?                              ."), QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    if (values == QMessageBox::Yes)
    {
        QSqlQuery query;
        query.exec("select * from LikeMusic");
...
void MusicPlayer::ClearALL_3_slot()//清除我的收藏
{
    int values = QMessageBox::information(this, (u8"清除我的收藏"), (u8"确定清除我的收藏? ."), QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    if (values == QMessageBox::Yes)
    {
        QSqlQuery query;
        query.exec("select * from CollectMusic");
...
void MusicPlayer::ClearALL_4_slot()//清除所有
{
    int values = QMessageBox::information(this, (u8"清除所有列表"), (u8"确定清除所有列表?                              ."), QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    if (values == QMessageBox::Yes)
    {
        QSqlQuery query;
...
void MusicPlayer::action1_slot()//添加菜单  添加至本地音乐
{
    ...
    QSqlQuery query;
    query.exec("select * from LocalMusic");
    QStringList list = QFileDialog::getOpenFileNames(this, (u8"文件"), currentFileName[1], (u8"音频文件(*.mp3)"));
    if (!list.isEmpty())
    {
        //进度条对话框
        ...
        progressdialog->setWindowTitle((u8"添加歌曲"));
        progressdialog->setLabelText((u8"正在添加,请稍后."));
        QPushButton* button = new QPushButton(this);
        QPixmap pixmap;
        pixmap.load(":/MusicPlayer/image/58d4d135e7bce713bd501050.jpg");
        QPalette palette;
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            item->setIcon(QIcon(":/MusicPlayer/image/Music_32px_1144946_easyicon.net.png"));
...
void MusicPlayer::action2_slot()//添加菜单  添加至我喜欢
{
    ListFocus();
    ui.stackedWidget->setCurrentIndex(0);
    pushbuttonstyle();
 ...
 {//进度条对话框
  ...
  for(int i=0;i<list.size();i++)
  {
      ...
      item2->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1170275_easyicon.net.png"));
      QString path=QDir::toNativeSeparators(list.at(i));
      if(!progressdialog->wasCanceled())
      {
          progressdialog->setValue(i);
...
void MusicPlayer::action3_slot()//添加菜单  添加至我的收藏
{
    ListFocus();
    ui.stackedWidget->setCurrentIndex(0);
    pushbuttonstyle();
    ...
    pushbuttonstyle_3();
     ...
     if(!list.isEmpty())
     {
      ...
      for(int i=0;i<list.size();i++)
      {
          ...
          item3->setIcon(QIcon(":/MusicPlayer/image/music_not_25.570093457944px_1171234_easyicon.net.png"));
          QString path=QDir::toNativeSeparators(list.at(i));
          if(!progressdialog->wasCanceled())
          {
          progressdialog->setValue(i);
...
void MusicPlayer::action4_1_slot()//设置菜单  透明度+5%
{
    QSqlQuery query;
...
void MusicPlayer::action4_2_slot()//设置菜单  透明度-5%
{
    QSqlQuery query;
    query.exec("select * from TRA_DATA");
    query.exec("delete from TRA_DATA");
...
void MusicPlayer::action5_1_slot()//设置默认壁纸1
{
    ListFocus();
    QString FileName = ":/MusicPlayer/image/background/bg8.jpg";
    QSqlQuery query;
...
void MusicPlayer::action5_2_slot()//设置默认壁纸2
{
    ListFocus();
    QString FileName = ":/MusicPlayer/image/background/bg7.jpg";
    QSqlQuery query;
    query.exec("delete from WALLPAPER_DATA");
    query.exec(QString("insert into WALLPAPER_DATA values('%1')").arg(FileName));
    ui.frame->setStyleSheet(QString("QFrame#frame{border-radius:5px;border-image: url(%1);}").arg(FileName));
}
void MusicPlayer::action5_4_slot()
{
    ListFocus();
    QString FileName = ":/MusicPlayer/image/background/bg3.jpg";
    QSqlQuery query;
    query.exec("delete from WALLPAPER_DATA");
    query.exec(QString("insert into WALLPAPER_DATA values('%1')").arg(FileName));
    ui.frame->setStyleSheet(QString("QFrame#frame{border-radius:5px;border-image: url(%1);}").arg(FileName));
...
void MusicPlayer::action5_3_slot()//设置菜单  设置自定义壁纸
{
    ListFocus();
    QString FileName = QFileDialog::getOpenFileName(this, (u8"文件"), currentFileName[0], (u8"图片文件(*jpg *png)"));
    if (!FileName.isEmpty())
    {
...
void MusicPlayer::action6_slot()//设置  关于
{
    ListFocus();
    ...
    QMessageBox::about(this, (u8"关于"), (
        u8"名称 : 我的音乐                         \n"
        u8"作者 : 王明泽                           \n"
        u8"介绍 : 我的音乐是个界面简洁,具有添加,播放,列表搜索,\n"
        u8"删除，播放记录，多选，清空列表，该表播放模式等基础功能,用户可按喜好设置皮肤,窗体透明度,\n"
        u8"使界面更加绚丽,支持右键菜单和列表菜单和快捷键,使用户操作更便利,\n"
        u8"极简模式,可以进行基础的播放暂停等功能,缩小窗口体积,使播放器可玩性更高\n"));



}
void MusicPlayer::action7_slot()//设置菜单  使用
{
    QMessageBox::about(this, (u8"使用"), (u8"右键 : 菜单栏               Esc : 退出程序\n"
        u8"双击 : 切换至极简模式         Ctrl-- : 透明度减\n"
        u8"空格 : 播放暂停              Ctrl++ : 透明度加\n"
        u8"向右键/数字键6 : 下一首       Ctrl+Q : 退出\n"
        u8"向左键/数字键4 : 上一首       Ctrl+V : 清除所有列表\n"
        u8"A : 切换列表                Ctrl+O : 添加至所有列表\n"
        u8"D : 切换列表                Ctrl+A : 添加至我喜欢列表\n"
        u8"W : 增加音量                Ctrl+Z : 添加至我的收藏\n"
        u8"S : 减少音量                Ctrl+X : 自定义皮肤\n"
        u8"Q : 定位                   Ctrl+M : 清除所有歌曲列表\n"
        u8"E : 切换播放模式             Ctrl+N : 清除我喜欢列表\n"
        u8"R : 添加我喜欢              Ctrl+B : 清除我的收藏\n"
        u8"X : 多选/取消多选                              .\n"));

    ListFocus();
}
void MusicPlayer::action8_slot()//主窗口右键菜单 关闭
{

    QCoreApplication::quit();

}
void MusicPlayer::action9_slot()//主窗口右键菜单 播放下一曲
{
    ...
    if (music->playlist() == playlist)
    {
        ...
        if (list1[currentIndex] == 0) {
            QListWidgetItem* myitem = new QListWidgetItem(QIcon(":/MusicPlayer/image/nextbo.png"), ui.listWidget_3->item(currentIndex)->text());
...
void MusicPlayer::action10_slot()//主窗口右键菜单 播放上一曲
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::action11_slot()//暂停
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::action11_1_slot()//播放
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::action12_1_slot()//主窗口右键菜单 顺序播放
{
...
void MusicPlayer::action12_2_slot()//主窗口右键菜单 随机播放
{

    ListFocus();
    ui.pushButton_9->setIcon(QIcon(":/MusicPlayer/image/image/play-random.png"));
    ui.pushButton_9->setToolTip((u8"随机播放"));
...
void MusicPlayer::action12_3_slot()//主窗口右键菜单 单曲循环
{

    ListFocus();
    ui.pushButton_9->setIcon(QIcon(":/MusicPlayer/image/image/loop-one.png"));
    ui.pushButton_9->setToolTip((u8"单曲循环"));
...
void MusicPlayer::on_pushButton_2_clicked()//播放/暂停
{
    ...
    else
    {
        ...
        ui.pushButton_2->setToolTip((u8"播放"));
        pause->setToolTip((u8"播放"));
        pause->setIcon(QIcon(":/MusicPlayer/image/zt13.png"));
        music->play();

    }
}

void MusicPlayer::on_pushButton_3_clicked()//播放下一曲
{
...
void MusicPlayer::on_pushButton_4_clicked()//播放上一曲
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::updatalistwidget_3(int value)//更新listWidget_3
{
...
void MusicPlayer::updatalistwidget_2(int value)//更新listWidget_2
{
...
void MusicPlayer::updatalistwidget(int value)//更新listWidget
{
...
void MusicPlayer::on_listWidget_3_doubleClicked(const QModelIndex& index)//双击播放playlist 本地音乐列表
{
...
void MusicPlayer::music_stateChang(QMediaPlayer::State state)//播放状态改变
{
...
void MusicPlayer::on_pushButton_15_clicked()//隐藏stackedWidget
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
    ...
    else
    {
        ...
        ui.pushButton_15->setIcon(QIcon(":/MusicPlayer/image/image/right.png"));
        ui.pushButton_15->setToolTip((u8"显示播放列表"));
...
void MusicPlayer::on_pushButton_9_clicked()//播放模式
{
...
void MusicPlayer::on_listWidget_2_doubleClicked(const QModelIndex& index)//双击播放playlist_2 我喜欢列表
{
...
void MusicPlayer::on_listWidget_doubleClicked(const QModelIndex& index)//双击播放playlist_3 我的收藏
{
...
void MusicPlayer::on_pushButton_7_clicked()//歌曲定位
{
...
void MusicPlayer::mousePressEvent(QMouseEvent* event)//拖拽 移动窗口
{
...
void MusicPlayer::mouseMoveEvent(QMouseEvent* event)//拖拽 移动窗口
{
    ListFocus();
...
void MusicPlayer::mouseReleaseEvent(QMouseEvent* event)//拖拽 移动窗口
{
    ListFocus();
    offset = QPoint();
    event->accept();
...
void MusicPlayer::mouseDoubleClickEvent(QMouseEvent*)//双击切换至极简模式
{
    setCursor(Qt::ArrowCursor);
...
void MusicPlayer::contextMenuEvent(QContextMenuEvent*)//主窗口右键菜单
{
...
void MusicPlayer::on_pushButton_17_clicked()//关闭
{
    QCoreApplication::quit();
}

void MusicPlayer::on_pushButton_16_clicked()//最小化
{
    ListFocus();
    showMinimized();
...
void MusicPlayer::on_pushButton_8_clicked()//隐藏verticalSlider
{
    ListFocus();
...
void MusicPlayer::changeVolume(int values)//改变音量
{
    if (values == 0)
    {

        ui.pushButton_8->setIcon(QIcon(":/MusicPlayer/image/image/none-music.png"));
        ui.pushButton_8->setStyleSheet(NoVoiceStyle());
    }
    else
    {
        ui.pushButton_8->setIcon(QIcon(":/MusicPlayer/image/music-voice.png"));
        ui.pushButton_8->setStyleSheet(VoiceStyle());
...
void MusicPlayer::on_listWidget_3_customContextMenuRequested(const QPoint& pos)//listWidget_3右键菜单 本地音乐列表
{
    ...
    Action1->setIcon(QIcon(":/MusicPlayer/image/zt13.png"));
    Action1->setText((u8"播放"));
    Action1_1->setIcon(QIcon(":/MusicPlayer/image/bofang1.png"));
    Action1_1->setText((u8"暂停"));
    Action2->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1170275_easyicon.net.png"));
    Action2->setText((u8"添加至我喜欢"));
    Action4->setIcon(QIcon(":/MusicPlayer/image/list_32px_1142913_easyicon.net.png"));
    Action4->setText((u8"添加至我的收藏"));
    Action7->setIcon(QIcon(":/MusicPlayer/image/duoxuan.png"));
    Action7->setText((u8"多选"));
    Action8->setIcon(QIcon(":/MusicPlayer/image/quxiao.png"));
    Action8->setText((u8"取消多选"));
    Action3->setIcon(QIcon(":/MusicPlayer/image/delete_32px_1168909_easyicon.net.png"));
    Action3->setText((u8"删除"));
    Action5->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    Action5->setText((u8"歌曲信息"));
    connect(Action1, &QAction::triggered, this, &MusicPlayer::Action1_slot);
    connect(Action1_1, &QAction::triggered, this, &MusicPlayer::action11_slot);
    connect(Action2, &QAction::triggered, this, &MusicPlayer::Action2_slot);
    connect(Action4, &QAction::triggered, this, &MusicPlayer::Action4_slot);
    connect(Action3, &QAction::triggered, this, &MusicPlayer::Action3_slot);
    connect(Action5, &QAction::triggered, this, &MusicPlayer::Action5_slot);
    connect(Action7, &QAction::triggered, this, &MusicPlayer::Action7_slot);
    connect(Action8, &QAction::triggered, this, &MusicPlayer::Action8_slot);
    if (ui.listWidget_3->selectionMode() == QListWidget::SingleSelection)
    {
        if (music->playlist() == playlist)
        {
...
void MusicPlayer::Action1_slot()//listWidget_3右键菜单 播放歌曲
{
...
void MusicPlayer::Action2_slot()//listWidget_3右键菜单 添加至我喜欢
{
    if (ui.listWidget_3->selectionMode() == QListWidget::SingleSelection)
    {
        model->setTable("LocalMusic");
        ...
        if (flag == 0)
        {
            ...
            ITEMM->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1101681_easyicon.net.png"));
        ...
        else
        {
            ...
            QMessageBox::information(this, (u8"提示"), (u8"该歌曲已添加 ."), QMessageBox::Yes);
    ...
    else if (ui.listWidget_3->selectionMode() == QListWidget::MultiSelection)
    {
        ...
        progressdialog->setMaximumSize(400, 120);
        progressdialog->setMinimumDuration(100);
        progressdialog->setWindowTitle((u8"添加至我喜欢"));
        progressdialog->setLabelText((u8"正在添加,请稍后."));
        QPushButton* button = new QPushButton(this);
        QPixmap pixmap;
        pixmap.load(":/MusicPlayer/image/58d4d135e7bce713bd501050.jpg");
        ...
        for (int i = 0; i < items.size(); i++)
        {
            ...
            if (flag == 0)
            {
                ...
                ITEMM1->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1101681_easyicon.net.png"));
...
void MusicPlayer::Action4_slot()//listWidget_3右键菜单 添加至我的收藏
{
    ...
    if (ui.listWidget_3->selectionMode() == QListWidget::SingleSelection)
    {
        ...
        QString Text = ui.listWidget_3->item(row)->text();
        QListWidgetItem* ITEMM1 = new QListWidgetItem;
        ITEMM1->setIcon(QIcon(":/MusicPlayer/image/music_not_25.570093457944px_1171234_easyicon.net.png"));
        ITEMM1->setText(QString("%1").arg(Text));
        ITEMM1->setToolTip(Text);
    ...
    else if (ui.listWidget_3->selectionMode() == QListWidget::MultiSelection)
    {
        ...
        progressdialog->setMaximumSize(400, 120);
        progressdialog->setMinimumDuration(100);
        progressdialog->setWindowTitle((u8"添加至我的收藏"));
        progressdialog->setLabelText((u8"正在添加,请稍后."));
        QPushButton* button = new QPushButton(this);
        QPixmap pixmap;
        pixmap.load(":/MusicPlayer/image/58d4d135e7bce713bd501050.jpg");
        ...
        for (int i = 0; i < items.size(); i++)
        {
            ...
            ITEMM2->setIcon(QIcon(":/MusicPlayer/image/music_not_25.570093457944px_1171234_easyicon.net.png"));
...
void MusicPlayer::Action7_slot()//多选模式
{
    ui.listWidget_3->setSelectionMode(QListWidget::MultiSelection);
}
void MusicPlayer::Action8_slot()//单选模式
{
...
void MusicPlayer::Action5_slot()//歌曲信息
{
    int row = ui.listWidget_3->currentIndex().row();
...
void MusicPlayer::Action3_slot()//listWidget_3右键菜单 删除
{
    ...
    else if (ui.listWidget_3->selectionMode() == QListWidget::MultiSelection)
    {
        ...
        progressdialog->setMaximumSize(400, 120);
        progressdialog->setMinimumDuration(100);
        progressdialog->setWindowTitle((u8"删除"));
        progressdialog->setLabelText((u8"正在删除,请稍后."));
        QPushButton* button = new QPushButton(this);
        QPixmap pixmap;
        pixmap.load(":/MusicPlayer/image/58d4d135e7bce713bd501050.jpg");
        QPalette palette;
...
void MusicPlayer::on_listWidget_2_customContextMenuRequested(const QPoint& pos)//listWidget_2右键菜单 我喜欢列表
{
    ...
    Action1_2->setIcon(QIcon(":/MusicPlayer/image/zt13.png"));
    Action1_2->setText((u8"播放"));
    Action1_2_1->setIcon(QIcon(":/MusicPlayer/image/bofang1.png"));
    Action1_2_1->setText((u8"暂停"));
    Action3_2->setIcon(QIcon(":/MusicPlayer/image/list_32px_1142913_easyicon.net.png"));
    Action3_2->setText((u8"添加至我的收藏"));

    Action2_2->setIcon(QIcon(":/MusicPlayer/image/delete_32px_1168909_easyicon.net.png"));
    Action2_2->setText((u8"移除"));
    Action4_2->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    Action4_2->setText((u8"歌曲信息"));
    Action6_2->setIcon(QIcon(":/MusicPlayer/image/duoxuan.png"));
    Action6_2->setText((u8"多选"));
    Action7_2->setIcon(QIcon(":/MusicPlayer/image/quxiao.png"));
    Action7_2->setText((u8"取消多选"));
    if (ui.listWidget_2->selectionMode() == QListWidget::SingleSelection)
    {
        if (music->playlist() == playlist_2)
        {
    ...
    connect(Action1_2, &QAction::triggered, this, &MusicPlayer::Action1_2_slot);
    connect(Action1_2_1, &QAction::triggered, this, &MusicPlayer::action11_slot);
    connect(Action2_2, &QAction::triggered, this, &MusicPlayer::Action2_2_slot);
    connect(Action3_2, &QAction::triggered, this, &MusicPlayer::Action3_2_slot);
    connect(Action4_2, &QAction::triggered, this, &MusicPlayer::Action4_2_slot);
    connect(Action6_2, &QAction::triggered, this, &MusicPlayer::Action6_2_slot);
    connect(Action7_2, &QAction::triggered, this, &MusicPlayer::Action7_2_slot);
...
void MusicPlayer::Action1_2_slot()//listWidget_2右键菜单   播放
{
...
void MusicPlayer::Action3_2_slot()//添加至我的收藏
{
...
void MusicPlayer::Action6_2_slot()//多选模式
{

    ui.listWidget_2->setSelectionMode(QListWidget::MultiSelection);
}
void MusicPlayer::Action7_2_slot()//单选模式
{
...
void MusicPlayer::Action4_2_slot()//歌曲信息
{
...
void MusicPlayer::Action2_2_slot()//listWidget_2右键菜单 删除
{
    if (ui.listWidget_2->selectionMode() == QListWidget::SingleSelection)
    {
    ...
    else if (ui.listWidget_2->selectionMode() == QListWidget::MultiSelection)
    {
        ...
        progressdialog->setMaximumSize(400, 120);
        progressdialog->setMinimumDuration(100);
        progressdialog->setWindowTitle((u8"移除"));
        progressdialog->setLabelText((u8"正在移除,请稍后."));
        QPushButton* button = new QPushButton(this);
        QPixmap pixmap;
        pixmap.load(":/MusicPlayer/image/58d4d135e7bce713bd501050.jpg");
        QPalette palette;
...
void MusicPlayer::on_listWidget_customContextMenuRequested(const QPoint& pos)//listWidget右键菜单  我的收藏
{
    ...
    Action1_3->setIcon(QIcon(":/MusicPlayer/image/zt13.png"));
    Action1_3->setText((u8"播放"));
    Action1_3_1->setIcon(QIcon(":/MusicPlayer/image/bofang1.png"));
    Action1_3_1->setText((u8"暂停"));
    Action2_3->setIcon(QIcon(":/MusicPlayer/image/delete_32px_1168909_easyicon.net.png"));
    Action2_3->setText((u8"移除"));
    Action3_3->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    Action3_3->setText((u8"歌曲信息"));
    Action5_3->setIcon(QIcon(":/MusicPlayer/image/duoxuan.png"));
    Action5_3->setText((u8"多选"));
    Action6_3->setIcon(QIcon(":/MusicPlayer/image/quxiao.png"));
    Action6_3->setText((u8"取消多选"));
    if (ui.listWidget->selectionMode() == QListWidget::SingleSelection)
    {
        if (music->playlist() == playlist_3)
        {
    ...
    connect(Action1_3, &QAction::triggered, this, &MusicPlayer::Action1_3_slot);
    connect(Action1_3_1, &QAction::triggered, this, &MusicPlayer::action11_slot);
    connect(Action2_3, &QAction::triggered, this, &MusicPlayer::Action2_3_slot);
    connect(Action3_3, &QAction::triggered, this, &MusicPlayer::Action3_3_slot);
    connect(Action5_3, &QAction::triggered, this, &MusicPlayer::Action5_3_slot);
    connect(Action6_3, &QAction::triggered, this, &MusicPlayer::Action6_3_slot);
...
void MusicPlayer::Action1_3_slot()//listWidget右键菜单 播放
{
...
void MusicPlayer::Action5_3_slot()//多选模式
{
    ui.listWidget->setSelectionMode(QListWidget::MultiSelection);
}
void MusicPlayer::Action6_3_slot()//单选模式
{
...
void MusicPlayer::Action3_3_slot()//歌曲信息
{
...
void MusicPlayer::Action2_3_slot()//listWidget右键菜单  删除
{
    if (ui.listWidget->selectionMode() == QListWidget::SingleSelection)
    {
        if (music->playlist() == playlist_3)
...
void MusicPlayer::on_pushButton_13_clicked()//极简模式
{
...
void MusicPlayer::Data_slot(int row, int position, QMediaPlaylist* list, QMediaPlayer::State state, QMediaPlaylist::PlaybackMode model, QList<int> Index)//接收数据
{
    ...
    if (list == playlist)
    {
        ...
        for (int i = 0; i < Index.size(); i++)
        {
            if (list1[Index[i]] == 0)
            {
                ...
                QListWidgetItem* myitem = new QListWidgetItem(QIcon(":/MusicPlayer/image/nextbo.png"), ui.listWidget_3->item(Index[i])->text());
...
void MusicPlayer::on_pushButton_14_clicked()//添加至我喜欢
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
    ...
    if (music->playlist() == playlist)
    {
        ...
        if (flag == 0)
        {
            ...
            QString Text = ui.listWidget_3->item(row)->text();
            QListWidgetItem* ITEMM2 = new QListWidgetItem;
            ITEMM2->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1101681_easyicon.net.png"));
            ITEMM2->setText(QString("%1").arg(Text));
...
void MusicPlayer::on_pushButton_18_clicked()//查找歌曲
{
    ...
    if (!text.isEmpty())
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            if (strstr(Name2, MusicName1) || strstr(Author2, MusicName1))
            {
                ...
                item20->setIcon(QIcon(":/MusicPlayer/image/sss.png"));
                item20->setText(Text);
...
void MusicPlayer::on_listWidget_4_doubleClicked(const QModelIndex& index)//双击播放ListWidget_4 搜索列表
{
...
void MusicPlayer::on_listWidget_4_customContextMenuRequested(const QPoint& pos)//listWidget_4 菜单 搜索列表
{
    ...
    Action->setIcon(QIcon(":/MusicPlayer/image/zt13.png"));
    Action->setText((u8"播放"));
    Action_A->setIcon(QIcon(":/MusicPlayer/image/bofang1.png"));
    Action_A->setText((u8"暂停"));
    Action1->setIcon(QIcon(":/MusicPlayer/image/like_outline_32px_1101681_easyicon.net.png"));
    Action1->setText((u8"添加至我喜欢"));
    Action2->setIcon(QIcon(":/MusicPlayer/image/list_32px_1074296_easyicon.net.png"));
    Action2->setText((u8"添加至我的收藏"));
    Action3->setIcon(QIcon(":/MusicPlayer/image/delete_32px_1168909_easyicon.net.png"));
    Action3->setText((u8"删除歌曲"));
    Action4->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    Action4->setText((u8"歌曲信息"));
    connect(Action, &QAction::triggered, this, &MusicPlayer::Action_slot);
    connect(Action_A, &QAction::triggered, this, &MusicPlayer::action11_slot);
    connect(Action1, &QAction::triggered, this, &MusicPlayer::Action_1_slot);
    connect(Action2, &QAction::triggered, this, &MusicPlayer::Action_2_slot);
    connect(Action3, &QAction::triggered, this, &MusicPlayer::Action_3_slot);
    connect(Action4, &QAction::triggered, this, &MusicPlayer::Action_4_slot);
...
void MusicPlayer::Action_slot()//播放
{
...
void MusicPlayer::Action_1_slot()//添加至我喜欢
{
    model->setTable("LocalMusic");
    model->select();
    int row = ui.listWidget_4->currentIndex().row();
...
void MusicPlayer::Action_2_slot()//添加至我的收藏
{
    int row = ui.listWidget_4->currentIndex().row();
    QString Text = ui.listWidget_3->item(load[row])->text();
    QListWidgetItem* ITEMM1 = new QListWidgetItem;
...
void MusicPlayer::Action_4_slot()//歌曲信息
{
...
void MusicPlayer::Action_3_slot()//删除
{
...
void MusicPlayer::CreateSortCut()//创建快捷键
{
    ...
    connect(play, &QShortcut::activated, this, &MusicPlayer::Key_Space_slot);
    QShortcut* next = new QShortcut(Qt::Key_Right, this);//下一首
    QShortcut* next1 = new QShortcut(Qt::Key_6, this);//下一首
    connect(next, &QShortcut::activated, this, &MusicPlayer::Next_slot);
    connect(next1, &QShortcut::activated, this, &MusicPlayer::Next_slot);
    QShortcut* pre = new QShortcut(Qt::Key_Left, this);//上一首
    QShortcut* pre1 = new QShortcut(Qt::Key_4, this);//上一首
    connect(pre, &QShortcut::activated, this, &MusicPlayer::Pre_slot);
    connect(pre1, &QShortcut::activated, this, &MusicPlayer::Pre_slot);
    QShortcut* nextlist = new QShortcut(Qt::Key_D, this);//转换列表
    connect(nextlist, &QShortcut::activated, this, &MusicPlayer::nextlist_slot);
    QShortcut* prelist = new QShortcut(Qt::Key_A, this);//转换列表
    connect(prelist, &QShortcut::activated, this, &MusicPlayer::prelist_slot);
    QShortcut* addV = new QShortcut(Qt::Key_W, this);//音量加
    connect(addV, &QShortcut::activated, this, &MusicPlayer::addV_slot);
    QShortcut* subV = new QShortcut(Qt::Key_S, this);//音量减
    connect(subV, &QShortcut::activated, this, &MusicPlayer::subV_slot);
    QShortcut* Locole = new QShortcut(Qt::Key_Q, this);//定位
    connect(Locole, &QShortcut::activated, this, &MusicPlayer::Locale_slot);
    QShortcut* change = new QShortcut(Qt::Key_E, this);//改变播放模式
    connect(change, &QShortcut::activated, this, &MusicPlayer::change_slot);
    QShortcut* love = new QShortcut(Qt::Key_R, this);//添加我喜欢
    connect(love, &QShortcut::activated, this, &MusicPlayer::love_slot);
    QShortcut* multiselect = new QShortcut(Qt::Key_X, this);//多选/取消多选
    connect(multiselect, &QShortcut::activated, this, &MusicPlayer::Multiselect_slot);



}
void MusicPlayer::Key_Space_slot()//播放/暂停
{
    ListFocus();
    if (music->state() == QMediaPlayer::PlayingState)
    {
...
void MusicPlayer::Multiselect_slot()//多选
{
...
void MusicPlayer::Next_slot()//下一首
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::nextlist_slot()//下一个播放列表
{
...
void MusicPlayer::prelist_slot()//上一个播放列表
{
...
void MusicPlayer::addV_slot()//增加音量
{
...
void MusicPlayer::subV_slot()//减少音量
{
...
void MusicPlayer::Locale_slot()//定位
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
...
void MusicPlayer::change_slot()//改变播放模式
{
    ...
    {
        ui.pushButton_9->setIcon(QIcon(":/MusicPlayer/image/loop.png"));
        ui.pushButton_9->setToolTip((u8"顺序播放"));
        playlist->setPlaybackMode(QMediaPlaylist::Loop);
        playlist_2->setPlaybackMode(QMediaPlaylist::Loop);
        playlist_3->setPlaybackMode(QMediaPlaylist::Loop);
...
void MusicPlayer::love_slot()//添加至我喜欢
{
    if (ui.listWidget->count() == 0 && ui.listWidget_2->count() == 0 && ui.listWidget_3->count() == 0 && ui.listWidget_4->count() == 0 && ui.listWidget_5->count() == 0) {
        return;
    }
    if (music->playlist() == playlist)
    {
        ...
        if (flag == 0)
        {
            ui.pushButton_14->setIcon(QIcon(":/MusicPlayer/image/Ilike.png"));
            ui.pushButton_14->setStyleSheet(IlikeStyle());
            ui.pushButton_14->setToolTip((u8"我喜欢"));
            int row = playlist->currentIndex();
            QString Text = ui.listWidget_3->item(row)->text();
...
void MusicPlayer::on_lineEdit_returnPressed()//输入框回车查找
{
    QString text = ui.lineEdit->text();
    if (!text.isEmpty())
    {
...
void MusicPlayer::PlaylistModel_slot(QMediaPlaylist::PlaybackMode model)//播放模式改变
{
...
void MusicPlayer::on_pushButton_19_clicked()
{
    ListFocus();
...
void MusicPlayer::on_pushButton_20_clicked()
{
    ListFocus();
...
inline void MusicPlayer::ListFocus() {
...
void MusicPlayer::ListItemToolTip()
{
...
void MusicPlayer::on_listWidget_5_doubleClicked(const QModelIndex& index)
{
...
void MusicPlayer::on_listWidget_5_customContextMenuRequested(const QPoint& pos)//播放历史
{
    ...
    play->setText((u8"播放"));
    puase->setIcon(QIcon(":/MusicPlayer/image/bofang1.png"));
    puase->setText((u8"暂停"));
    information->setIcon(QIcon(":/MusicPlayer/image/inf.png"));
    information->setText((u8"歌曲信息"));

    Clear->setText((u8"清除播放记录"));
    Clear->setIcon(QIcon(":/MusicPlayer/image/quxiao.png"));
    ...
    connect(play, &QAction::triggered, this, &MusicPlayer::playMusic_slot);
    connect(puase, &QAction::triggered, this, &MusicPlayer::action11_slot);
    connect(information, &QAction::triggered, this, &MusicPlayer::listWidget_5_info);
    connect(Clear, &QAction::triggered, this, &MusicPlayer::clearListWidget_slot);
    Menu->exec(QCursor::pos());
    delete Menu;
    delete  play;
    delete  puase;
...
void MusicPlayer::playMusic_slot()
{
    if (music->playlist() == playlist_4)
    {
        if (ui.listWidget_5->currentIndex().row() == playlist_4->currentIndex())
...
void MusicPlayer::listWidget_5_info()
{
    {
...
void MusicPlayer::clearListWidget_slot()
{
    ClearHistory_Slot();
}
